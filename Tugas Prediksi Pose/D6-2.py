...
if not cap.isOpened():
    raise RuntimeError("Kamera tidak bisa dibuka. Pastikan DroidCam aktif!")
...
# ✅ Fungsi perhitungan sudut custom
def find_angle_custom(lmList, p1, p2, p3):
    try:
        x1, y1 = lmList[p1][1:3]
        x2, y2 = lmList[p2][1:3]
        x3, y3 = lmList[p3][1:3]
    ...
    radians = np.arccos(np.dot(ab, cb) / (np.linalg.norm(ab) * np.linalg.norm(cb) + 1e-8))
...
while True:
    ...
    if lmList:
        if MODE == "squat":
            angL = find_angle_custom(lmList, 23, 25, 27)
            angR = find_angle_custom(lmList, 24, 26, 28)
